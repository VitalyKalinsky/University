...
public class MyTreeSet<K, V> {
    ...
    private V putHelper(Node node, K key, V value) {
        if(node.key != key) {
            ...
            System.out.println("cannot add node with key " + key + " and value " + value);
            return null;
        }
    }

    public K contains(V value){
       return containsLER(root, value);
    }

    private K containsLER(Node node, V value){
        if (node.left != null) return containsLER(node.left, value);
        if(node.value == value) return node.key;
        if (node.right != null) return containsLER(node.right, value);
        return null;
    }
