...
public class HomeWork7 {
    ...
    public static List<String> methodOne(List<String> names) {
        return names.stream().filter(el -> el.contains("King")).collect(Collectors.toList());
    }
    ...
    public static List<String> methodTwo(List<String> names) {
        return names.stream()
                .map(el -> el.substring(el.indexOf(' ') + 1))
                .distinct()
    ...
    /**
     * Метод должен вернуть имена людей, чья фамилия начинается с K или S в формате имя + первая буква фомилии
    ...
    public static List<String> methodSeven(List<String> names) {
        return names.stream()
                .filter(el -> el.substring(el.indexOf(' ') + 1).startsWith("K") ||
                        el.substring(el.indexOf(' ') + 1).startsWith("S"))
                .map(el -> el.substring(0, el.indexOf(' ') + 2) + ".")
                .distinct()
                .sorted()
                .collect(Collectors.toList());
    }
    ...
    public static void printTestCase(List<String> exp, List<String> act) {
        ...
        if (Objects.equals(exp, act)) System.out.print("Yes");
